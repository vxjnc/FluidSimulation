...
fn calc_distance(cell: vec2f, src: vec2f, radius: f32) -> f32 {
    if inject.form == 0u {
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
    ...
    // --- velocity ---
    if in_phys && (inject.mode_mask & 1u) != 0u {
        let phys_cell = vec2f(f32(x), f32(y));
        let src_phys = vec2f(inject.x, inject.y);
        let eff_r = select(inject.radius, 2.0, inject.form == 1u);
        let d = calc_distance(phys_cell, src_phys, inject.radius);
        if d < eff_r {
            let falloff = 1.0 - d / eff_r;
            velocity[idx_phys(x, y)] += vec2f(inject.vx, inject.vy) * falloff;
        }
    }

    // --- dye ---
    if in_dye && (inject.mode_mask & 2u) != 0u {
        ...
        let src_dye = vec2f(inject.x, inject.y) * scale;
        let eff_r = select(inject.radius * scale.x, 2.0, inject.form == 1u);
        let d = calc_distance(dye_cell, src_dye, inject.radius * scale.x);
        if d < eff_r {
            let falloff = 1.0 - d / eff_r;
            dye[idx_dye(x, y)] = min(dye[idx_dye(x, y)] + vec4f(inject.color.rgb * falloff, falloff), vec4f(1.0));
        }
    }
}
