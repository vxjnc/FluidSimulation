...
void SaveManager::load(const std::filesystem::path& path, FluidSim& sim, std::vector<FluidSource>& sources,
    ...
    sources =
        std::ranges::to<std::vector>(file.sources | std::views::transform(&FsimFluidSource::toFluidSource));
